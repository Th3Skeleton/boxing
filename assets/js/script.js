    ...
    fetch("/assets/json/miis.json")
        ...
        .then((data) => {
            ...
            for (let i = 0; i < mii.length; i++) {
                ...
                btn.innerHTML = "<img src='assets/img/" + mii[i].name + ".png'>" + mii[i].name;
                btn.id = "miiBtn";
                btn.onclick = function () {
                    resdiv.innerHTML = mii[i].name;
                    resimg.src = "assets/img/" + mii[i].name + ".png";
                };
                gen.appendChild(btn);

                if (mii[i].isPicked == true) {
                    btn.disabled = true;
                    btn.innerHTML = "<img src='assets/img/" + mii[i].name + ".png'>" + mii[i].name + " [PICKED]";
                }
            }
